...
/**
 * CUSTOM CHECKOUT BUTTON INTERCEPTOR
 * Listens for any click on a button with name="checkout"
 */
document.addEventListener('click', function(event) {
  // 1. Check if the clicked element (or its icon/text child) is the checkout button
  const checkoutBtn = event.target.closest('button[name="checkout"], input[name="checkout"]');
  
  // If it's not the checkout button, do nothing and let the click happen normally
  if (!checkoutBtn) return;

  // 2. Stop Shopify from immediately redirecting to checkout
  ...
  // 4. Resume checkout once your logic is finished
  // Find the exact form this button is associated with
  const formId = checkoutBtn.getAttribute('form');
  const cartForm = formId ? document.getElementById(formId) : checkoutBtn.closest('form');

  if (cartForm) {
    // Add a hidden input to ensure Shopify processes it as a "checkout" action, 
    // not just a standard cart "update" action
    ...
    // Submit the form to proceed to the payment page
    cartForm.submit();
  } else {
    console.error("Cart form not found for the checkout button.");
  }
});
